...
impl<TX: DbTx, Spec: Send + Sync> DatabaseProvider<TX, Spec> {
        ...
        ) -> ProviderResult<Option<B>>,
    {
        ...
        let shadows = self.shadows_by_block(block_number.into())?;
        // Get the block body
        //
        ...
        construct_block(header, body, senders, ommers, withdrawals, requests, shadows)
    }
    ...
    for DatabaseProvider<TX, Spec>
{
    ...
    fn insert_block(
        ...
        if let Some(shadows) = block.block.body.shadows {
            // if !shadows.is_empty() { // ALWAYS STORE
            self.tx.put::<tables::BlockShadows>(block_number, StoredBlockShadows { shadows })?;
            durations_recorder.record_relative(metrics::Action::InsertBlockShadows);
            // }
        }
