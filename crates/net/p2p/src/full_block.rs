...
use reth_primitives::{constants::EMPTY_SHADOWS_ROOT, BlockBody, GotExpected, Header, SealedBlock, SealedHeader};
use std::{
    cmp::Reverse,
...
fn ensure_valid_body_response(
    ...
    let shadows_root = match &block.shadows {
        Some(shadows) => reth_primitives::proofs::calculate_shadows_root(&shadows),
        None => EMPTY_SHADOWS_ROOT,
    };
    if shadows_root != header.shadows_root {
        return Err(ConsensusError::BodyShadowsRootDiff(
            GotExpected { got: shadows_root, expected: header.shadows_root }.into(),
        ));
    }
